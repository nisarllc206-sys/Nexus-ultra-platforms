          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={s.card}>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
                ...
                <div>
                  <button
                    style={{ ...s.btn("linear-gradient(90deg,#a78bfa,#38bdf8)"), opacity: aiLoading ? 0.7 : 1, minWidth: 140 }}
                    disabled={aiLoading}
              ...
              {aiOutput && (
                <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: 18, border: `1px solid ${clr.border}` }}>
                  <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 8 }}>✅ Generated Content</div>
                  <div style={{ fontSize: 14, lineHeight: 1.7, whiteSpace: "pre-wrap", color: "#e2e8f0" }}>{aiOutput}</div>
                  <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                    <button style={s.btn(clr.purple, true)} onClick={() => { navigator.clipboard?.writeText(aiOutput); notify("📋 Content copied!"); }}>📋 Copy</button>
                    <button style={s.btn(clr.blue, true)} onClick={() => setAiOutput("")}>🗑️ Clear</button>
                  </div>
                </div>
              )}
            </div>
