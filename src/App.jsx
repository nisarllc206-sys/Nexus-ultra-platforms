...
export default function App() {
  ...
  const generateContent = async () => {
    const storedKey = localStorage.getItem("nw_api_key") || "";
    if (!storedKey) {
      setAiOutput("⚠️ No API key found. Configure your Anthropic API key in NEXUS WALLS settings (open public/index.html).");
      return;
    }
    setAiLoading(true);
    setAiOutput("");
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={s.card}>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginBottom: 16, flexWrap: "wrap" }}>
                ...
                <button
                  style={{ ...s.btn("linear-gradient(90deg,#a78bfa,#38bdf8)"), padding: "10px 28px", opacity: aiLoading ? 0.6 : 1 }}
              ...
              {aiOutput && (
                <div style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${clr.border}`, borderRadius: 12, padding: "18px 20px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <span style={{ fontSize: 12, color: clr.purple, fontWeight: 700 }}>✅ Generated Content</span>
                    <button style={s.btn(clr.blue, true)} onClick={() => { navigator.clipboard?.writeText(aiOutput); notify("📋 Content copied!"); }}>Copy</button>
                  </div>
                  <div style={{ fontSize: 14, color: "#e2e8f0", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{aiOutput}</div>
                </div>
              )}
            </div>
          </div>
